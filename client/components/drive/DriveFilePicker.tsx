...
function ImageLightbox({ src, name, onClose }: { src: string; name: string; onClose: () => void }) {
  ...
  return (
    <div
      className="fixed inset-0 z-[200] bg-black/80 backdrop-blur-sm flex flex-col items-center justify-center p-6 animate-in fade-in duration-200"
      onClick={onClose}
    >
      <div
        className="relative max-w-3xl w-full bg-card rounded-3xl overflow-hidden shadow-2xl border border-white/10"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/10">
          <p className="text-sm font-black truncate text-foreground">{name}</p>
          <button
            onClick={onClose}
        ...
        <div className="p-4 bg-black/20 flex items-center justify-center min-h-[300px]">
          <img
            src={src}
            alt={name}
...
export function DriveFilePicker({
  ...
  // ── Picker content ────────────────────────────────────────────────────────
  const pickerContent = (
    <div className="flex flex-col w-full h-full bg-background/50 border border-white/10 rounded-[2rem] overflow-hidden">
      {/* Search bar */}
      ...
      {isNotConnected ? (
        ...
        <ScrollArea className="max-h-[420px] min-h-[300px]">
            ...
            {data?.files?.map((file) => {
              ...
              return (
                <div
                  key={file.id}
                  className={cn(
                    "w-full flex items-center gap-4 px-5 py-3.5 rounded-2xl text-left transition-all group relative overflow-hidden cursor-pointer",
                    isSelected
                ...
                >
                  ...
                  <div className="shrink-0 w-10 h-10 relative">
                    {isImg && file.thumbnailLink ? (
                      ...
                      <span className="text-3xl group-hover:scale-110 transition-transform block text-center leading-10">
                  ...
                  {isImg && file.webViewLink && (
                    <a
                      ...
                      className={cn(
                        ...
                        isSelected ? "text-white/70 hover:bg-white/20" : "text-muted-foreground hover:bg-muted"
                      )}
                      title="Open in Drive"
                    >
      ...
      <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-500">
        ...
        {inline ? (
          ...
          <Popover open={open} onOpenChange={setOpen}>
            ...
            <PopoverContent
              className="w-[420px] p-0 glass-card rounded-[2rem] border-white/10 overflow-hidden shadow-2xl animate-in slide-in-from-top-4 duration-300"
              align="start"
            >
              {pickerContent}
            </PopoverContent>
